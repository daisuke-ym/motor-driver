...
// サーボ速度
uint8_t SVWAIT[] = {3, 5}; // ボタン押しっぱなし時のwait時間(ms)
uint8_t SVSpeed = sizeof(SVWAIT) / sizeof(uint8_t); // SVWAIT[] 配列の要素数
uint8_t SVSpeedIndex = 0; // 現在どの段階にいるか

// サーボグループ（ ボタンで選択）
uint8_t SVGROUP = 0;
uint8_t SVGROUPNUM = 1;
...
void loop() {
  ...
  if (PS_DBR != PS_DBR0)  {
    switch (PS_DBR) {
      case 0x08: // 三角ボタン
        analogOut(7, CW, PWM[PWMIndex]);
        break;
      case 0x04: // 四角ボタン
        analogOut(7, CCW, PWM[PWMIndex]);
        break;
      case 0x02: // バツボタン
        analogOut(8, CW, PWM[PWMIndex]);
        break;
      case 0x01: // 丸ボタン
        analogOut(8, CCW, PWM[PWMIndex]);
        break;
      case 0x00:
        analogOut(7, CW, 0);
        analogOut(8, CW, 0);
  ...
  if (PS_LR != 0) {
    ...
    if ((PS_LR & 0x01) != 0) { // R3
      SV_ANGLE[2]++;
      if (SV_ANGLE[2] >= SV_AMAX[2]) SV_ANGLE[2] = SV_AMAX[2];
      SV3.write(SV_ANGLE[2]);
#if DEBUGLEVEL > 3
      Serial.print("SV3: ");
      Serial.print(SV_ANGLE[2]);
      Serial.println();
#endif
    }
    if ((PS_LR & 0x10) != 0) { // L3
      SV_ANGLE[2]--;
      if (SV_ANGLE[2] <= SV_AMIN[2]) SV_ANGLE[2] = SV_AMIN[2];
      SV3.write(SV_ANGLE[2]);
#if DEBUGLEVEL > 3
      Serial.print("SV3: ");
      Serial.print(SV_ANGLE[2]);
...
void readPS4() {
  ...
  if (PS4.connected()) {
    ...
    // OPTIONS ボタンでサーボの速度を変える
    if (PS4.getButtonClick(OPTIONS)) {
      SVSpeedIndex++;
      if (SVSpeedIndex >= SVSpeed) {
        SVSpeedIndex = 0;
      }
      // サーボ速度により LED1 の色を変える
      switch (SVSpeedIndex) {
        case 0:
          LED(0, LED_CYAN);
          break;
...
#if DEBUGLEVEL > 5
      Serial.print("SVSpeedIndex: ");
      Serial.println(SVSpeedIndex);
#endif
    }
  }

  // コントローラの接続が切れた時，全てのモータを止める
  if (!PS4.connected()) {
    LED(0, LED_RED);
